...
public class SortingPractice {
    ...
    static int[] iterativeMergeSort(int[] array) {
        int numOfmerge = (int) (Math.log(array.length) / Math.log(2));
        int numOfarrays = numOfmerge + 1;
        int masterArray[][][] = new int[numOfarrays][][];
        System.out.println("\n\nArray length equals: " + array.length +
                "\nNumber of merges: " + numOfmerge);
        // TO SPLIT INTO SINGULAR LENGTH IS 8
        int[][] singular = new int[array.length][];
        for(int i = 0; i < array.length; i++) {
            singular[i] = slice(array,i,i+1);
        }
        // Assigns first 2d array in 3d array as the singular elements
        masterArray[0] = singular;

        // Will create the pairs, sort them, join them, and repeat until it reaches one array.
        // Starts the merge array that will be used to store the paired elements
        int mergeLength = array.length/2;
        for(int i = 0; i < numOfmerge; i++) {
            int[][] merge = new int[mergeLength][];
            int j = 0;
            // Pairs the arrays
            for(int z = 0; z < merge.length; z++) {
                merge[z] = merge(masterArray[i][j],masterArray[i][j+1]);
                j += 2;
            }
            // Assigns the complete stage of the merged array to the master array
            masterArray[i+1] = merge;
            // Prepares merge array length for next merge making length cut in half
            mergeLength = mergeLength/2;
        }
        /**
         * @PSUEDOCODE
         *
        //TO MERGE INTO PAIRS LENGTH IS 4
        int[][] slicedArrays1 = new int[array.length/2 + array.length%2][];
        int j = 0;
        for(int i = 0; i < slicedArrays1.length; i++) {
            slicedArrays1[i] = merge(singular[j],singular[j+1]);
            j += 2;
        }

        //TO MERGE INTO QUADS LENGTH IS 2
        int[][] slicedArrays2 = new int[slicedArrays1.length/2][];
        j = 0;
        for(int i = 0; i < slicedArrays2.length; i++) {
            slicedArrays2[i] = merge(slicedArrays1[j],slicedArrays1[j+1]);
            j += 2;
        }

        //TO MERGE INTO ONE LENGTH IS 1
        int[][] slicedArrays3 = new int[slicedArrays2.length/2][];
        j = 0;
        for(int i = 0; i < slicedArrays3.length; i++) {
            slicedArrays3[i] = merge(slicedArrays2[j],slicedArrays2[j+1]);
            j += 2;
        }
         */
        // Last merge pair will be the complete sorted array
        array = masterArray[masterArray.length-1][0];
        // Prints array analytics
        System.out.println("Sorted array length: " + array.length +
                "\nSorted: " + isSorted(array));
        //array = slicedArrays3[0];
        return array;
    ...
    static void test() {
        ...
        while (test < TESTS && testOutcome) {
            ...
            testArray = iterativeMergeSort(testArray);
